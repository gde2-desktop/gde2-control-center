...
system_tile_new (const gchar *desktop_item_id, const gchar *title)
{
	...
	Gde2DesktopItem *desktop_item = NULL;
	gchar            *image_id     = NULL;
	...
	desktop_item = libslab_gde2_desktop_item_new_from_unknown_id (desktop_item_id);

	if (desktop_item) {
		image_id = g_strdup (gde2_desktop_item_get_localestring (desktop_item, "Icon"));
		uri      = g_strdup (gde2_desktop_item_get_location (desktop_item));

		if (title)
			header_txt = g_strdup (title);
		else
			header_txt = g_strdup (
				gde2_desktop_item_get_localestring (desktop_item, "Name"));
	}
