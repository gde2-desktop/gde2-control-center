...
static StartupStatus get_desktop_item_startup_status (Gde2DesktopItem *);
	...
	GtkIconSize image_size, gboolean show_generic_name)
{
	...
	if (
		desktop_item &&
		gde2_desktop_item_get_entry_type (desktop_item) == GDE2_DESKTOP_ITEM_TYPE_APPLICATION
	)
		uri = gde2_desktop_item_get_location (desktop_item);
...
application_tile_finalize (GObject *g_object)
{
	...
	if (priv->desktop_item) {
		gde2_desktop_item_unref (priv->desktop_item);
		priv->desktop_item = NULL;
	}
	if (priv->image_id) {
...
application_tile_setup (ApplicationTile *this)
{
	...
	priv->image_id = g_strdup (gde2_desktop_item_get_localestring (priv->desktop_item, "Icon"));
	image = themed_icon_new (priv->image_id, priv->image_size);

	name = gde2_desktop_item_get_localestring (priv->desktop_item, "Name");
	desc = gde2_desktop_item_get_localestring (priv->desktop_item, "GenericName");
	comment = gde2_desktop_item_get_localestring (priv->desktop_item, "Comment");	
...
/* make help action */
	...
	if (gde2_desktop_item_get_string (priv->desktop_item, "DocPath")) {
...
add_to_startup_list (ApplicationTile *this)
{
	...
	desktop_item_filename =
		g_filename_from_uri (gde2_desktop_item_get_location (priv->desktop_item), NULL,
		NULL);
	...
	src_uri = gde2_desktop_item_get_location (priv->desktop_item);
...
get_desktop_item_startup_status (Gde2DesktopItem *desktop_item)
{
	gchar *filename;
	gchar *basename;
	...
	filename = g_filename_from_uri (gde2_desktop_item_get_location (desktop_item), NULL, NULL);
	if (!filename)
	...
	/* gde2-session currently checks these dirs also. see startup-programs.c */
	if (retval != APP_NOT_ELIGIBLE)
	{
		global_dirs = g_get_system_data_dirs();
		for(x=0; global_dirs[x]; x++)
		{
			global_target = g_build_filename (global_dirs[x], "gde2", "autostart", basename, NULL);
			if (g_file_test (global_target, G_FILE_TEST_EXISTS))
			{
				retval = APP_NOT_ELIGIBLE;
				g_free (global_target);
