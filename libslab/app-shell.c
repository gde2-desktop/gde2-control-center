...
#define CC_SCHEMA "org.gde2.control-center"
...
static void generate_category (const char * category, Gde2MenuTreeDirectory * root_dir, AppShellData * app_data, gboolean recursive);
static void generate_launchers (Gde2MenuTreeDirectory * root_dir, AppShellData * app_data,
	CategoryData * cat_data, gboolean recursive);
static void generate_new_apps (AppShellData * app_data);
static void insert_launcher_into_category (CategoryData * cat_data, Gde2DesktopItem * desktop_item,
...
static void gde2menu_tree_changed_callback (Gde2MenuTree * tree, gpointer user_data);
...
static void
gde2menu_tree_changed_callback (Gde2MenuTree * old_tree, gpointer user_data)
{
	/*
	This method only gets called on the first change (gde2menu appears to ignore subsequent) until
...
{
	Gde2MenuTreeDirectory *root_dir;
	GSList *contents, *l;
	gboolean need_misc = FALSE;

	if (!app_data->tree)
	{
		app_data->tree = gde2menu_tree_lookup (app_data->menu_name, GDE2MENU_TREE_FLAGS_NONE);
		gde2menu_tree_add_monitor (app_data->tree, gde2menu_tree_changed_callback, app_data);
	}
	root_dir = gde2menu_tree_get_root_directory (app_data->tree);
	if (root_dir)
		contents = gde2menu_tree_directory_get_contents (root_dir);
	else
		contents = NULL;
	if (!root_dir || !contents)
	{
		...
		exit (1);	/* Fixme - is there a GDE2/GTK way to do this. */
	}
	...
	for (l = contents; l; l = l->next)
	{
		...
		case GDE2MENU_TREE_ITEM_DIRECTORY:
			category = gde2menu_tree_directory_get_name ((Gde2MenuTreeDirectory*)item);
			generate_category(category, (Gde2MenuTreeDirectory*)item, app_data, TRUE);
			break;
	...
	gde2menu_tree_item_unref (root_dir);
...
static void
generate_category (const char * category, Gde2MenuTreeDirectory * root_dir, AppShellData * app_data, gboolean recursive)
{
	CategoryData *data;
	/* This is not needed. Gde2Menu already returns an ordered, non duplicate list
	GList *list_entry;
	list_entry =
	...
	*/
		...
		app_data->categories_list =
			/* use the gde2menu order instead of alphabetical */
...
static gboolean
check_specific_apps_hack (Gde2DesktopItem * item)
{
	static const gchar *COMMAND_LINE_LOCKDOWN_SCHEMA = "org.gde2.lockdown";
	...
	/* discard xscreensaver if gde2-screensaver is installed */
	if ((exec && !strcmp (exec, "xscreensaver-demo"))
		&& (path = g_find_program_in_path ("gde2-screensaver-preferences")))
	{
		g_free (path);
		return TRUE;
	}
	...
	if (command_line_lockdown)
	{
		const gchar *categories =
			gde2_desktop_item_get_string (item, GDE2_DESKTOP_ITEM_CATEGORIES);
		if (g_strrstr (categories, COMMAND_LINE_LOCKDOWN_DESKTOP_CATEGORY))
		{
			/* printf ("eliminating %s\n", gde2_desktop_item_get_location (item)); */
			return TRUE;
		}
	}
...
static void
generate_launchers (Gde2MenuTreeDirectory * root_dir, AppShellData * app_data, CategoryData * cat_data, gboolean recursive)
{
	Gde2DesktopItem *desktop_item;
	...
	for (l = contents; l; l = l->next)
	{
		...
		{
		case GDE2MENU_TREE_ITEM_DIRECTORY:
			/* g_message ("Found sub-category %s", gde2menu_tree_directory_get_name (l->data)); */
			if (recursive)
				generate_launchers (l->data, app_data, cat_data, TRUE);
			break;
		case GDE2MENU_TREE_ITEM_ENTRY:
			/* g_message ("Found item name is:%s", gde2menu_tree_entry_get_name (l->data)); */
			desktop_file = gde2menu_tree_entry_get_desktop_file_path (l->data);
			if (desktop_file)
			{
				...
				/* Fixme - make sure it's safe to store this without duping it. As far as I can tell it is
				   safe as long as I don't hang on to this anylonger than I hang on to the Gde2MenuTreeEntry*
			...
			desktop_item = gde2_desktop_item_new_from_file (desktop_file, 0, NULL);
			if (!desktop_item)
			{
				g_critical ("Failure - gde2_desktop_item_new_from_file(%s)",
					    desktop_file);
				break;
			}
			if (!check_specific_apps_hack (desktop_item))
				insert_launcher_into_category (cat_data, desktop_item, app_data);
			gde2_desktop_item_unref (desktop_item);
			break;
		default:
			break;
		}
...
generate_new_apps (AppShellData * app_data)
{
	...
	all_apps_file_name = g_build_filename (g_get_user_config_dir (), "gde2", "ab-newapps.txt", NULL);
	...
	if (!g_file_get_contents (all_apps_file_name, &all_apps, NULL, &error))
	{
		...
		for (categories = app_data->categories_list; categories; categories = categories->next)
		{
			...
			for (launchers = data->launcher_list; launchers; launchers = launchers->next)
			{
				...
				const gchar *uri = gde2_desktop_item_get_location (item);
...
static void
insert_launcher_into_category (CategoryData * cat_data, Gde2DesktopItem * desktop_item,
	AppShellData * app_data)
{
	GtkWidget *launcher;
	...
	launcher =
		application_tile_new_full (gde2_desktop_item_get_location (desktop_item),
		app_data->icon_size, app_data->show_tile_generic_name);
	gtk_widget_set_size_request (launcher, SIZING_TILE_WIDTH, -1);

	filepath =
		g_strdup (gde2_desktop_item_get_string (desktop_item, GDE2_DESKTOP_ITEM_EXEC));
	...
	*/
	...
	/* use alphabetical order instead of the gde2menu order. We group all sub items in each top level
