...
#include "gde2-utils.h"
#include "libslab-utils.h"
#include "bookmark-agent.h"

#define GNOME_MAIN_MENU_SCHEMA         "org.gde2.gnome-main-menu.file-area"
...
document_tile_new (BookmarkStoreType bookmark_store_type, const gchar *in_uri, const gchar *mime_type, time_t modified)
{
	 ...
	 * equivalent to gde2_vfs_uri_is_local, but that method will stat the file and
...
load_image (DocumentTile *tile)
{
	...
	Gde2DesktopThumbnailFactory *thumbnail_factory;
	GIcon *icon;
	...
	icon_id = gde2_desktop_thumbnail_factory_lookup (thumbnail_factory, TILE (tile)->uri, priv->modified);
