...
gchar *
get_package_name_from_desktop_item (Gde2DesktopItem * desktop_item)
{
	gchar *argv[6];
	gchar *package_name;
	gint retval;
	...
	argv[2] = "--qf";
	argv[3] = "%{NAME}";
	argv[4] = g_filename_from_uri (gde2_desktop_item_get_location (desktop_item), NULL, NULL);
	argv[5] = NULL;
...
gboolean
open_desktop_item_exec (Gde2DesktopItem * desktop_item)
{
	GError *error = NULL;
	...
	gde2_desktop_item_launch (desktop_item, NULL, GDE2_DESKTOP_ITEM_LAUNCH_ONLY_ONE | GDE2_DESKTOP_ITEM_LAUNCH_DO_NOT_REAP_CHILD, &error);

	if (error)
	{
		g_warning ("error launching %s [%s]\n",
			gde2_desktop_item_get_location (desktop_item), error->message);
...
gboolean
open_desktop_item_help (Gde2DesktopItem * desktop_item)
{
	...
	doc_path = gde2_desktop_item_get_string (desktop_item, "DocPath");
