...
Gde2DesktopItem *
libslab_gde2_desktop_item_new_from_unknown_id (const gchar *id)
{
	...
	item = gde2_desktop_item_new_from_uri (id, 0, & error);
	...
	item = gde2_desktop_item_new_from_file (id, 0, & error);
	...
	item = gde2_desktop_item_new_from_basename (id, 0, & error);
	...
	if (basename) {
		...
		item = gde2_desktop_item_new_from_basename (basename, 0, &error);
...
gboolean
libslab_gde2_desktop_item_launch_default (Gde2DesktopItem *item)
{
	GError *error = NULL;
	...
	gde2_desktop_item_launch (item, NULL, GDE2_DESKTOP_ITEM_LAUNCH_ONLY_ONE, & error);

	if (error) {
		g_warning ("error launching %s [%s]\n",
			gde2_desktop_item_get_location (item), error->message);
...
gchar *
libslab_gde2_desktop_item_get_docpath (Gde2DesktopItem *item)
{
	gchar *path;

	path = g_strdup (gde2_desktop_item_get_localestring (item, GDE2_DESKTOP_ITEM_DOC_PATH));

	if (! path)
		path = g_strdup (gde2_desktop_item_get_localestring (item, ALTERNATE_DOCPATH_KEY));
...
gboolean
libslab_gde2_desktop_item_open_help (Gde2DesktopItem *item)
{
	gchar *doc_path;
	gchar *help_uri;
	...
	doc_path = libslab_gde2_desktop_item_get_docpath (item);
...
{
	Gde2DesktopItem *d_item;
	const gchar      *categories;

	gboolean is_terminal = FALSE;


	d_item = libslab_gde2_desktop_item_new_from_unknown_id (uri);

	if (! d_item)
		return FALSE;

	categories = gde2_desktop_item_get_string (d_item, GDE2_DESKTOP_ITEM_CATEGORIES);

	is_terminal = (categories && strstr (categories, DESKTOP_ITEM_TERMINAL_EMULATOR_FLAG));

	gde2_desktop_item_unref (d_item);
...
libslab_desktop_item_is_logout (const gchar *uri)
{
	...
	is_logout = strstr ("Logout", gde2_desktop_item_get_string (d_item, GDE2_DESKTOP_ITEM_NAME)) != NULL;
...
libslab_desktop_item_is_lockscreen (const gchar *uri)
{
	...
	is_logout = strstr ("Lock Screen", gde2_desktop_item_get_string (d_item, GDE2_DESKTOP_ITEM_NAME)) != NULL;
...
static Gde2DesktopThumbnailFactory *thumbnail_factory;
