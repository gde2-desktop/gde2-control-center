...
#include <libwindow-settings/gde2-wm-manager.h>
#include <string.h>
...
{
	Gde2ThemeMetaInfo* theme;
	time_t mtime = -1;

	theme = gde2_theme_meta_info_find(name);
	if (theme != NULL)
	{
		GFile* file;
		GFileInfo* file_info;
...
static void theme_thumbnail_update(GdkPixbuf* pixbuf, gchar* theme_name, AppearanceData* data, gboolean cache)
{
	...
	if (theme_find_in_model(model, theme_name, &iter))
	{
		...
		if (cache && (mtime = theme_get_mtime(theme_name)) != -1)
		{
			...
			gde2_desktop_thumbnail_factory_save_thumbnail(data->thumb_factory, pixbuf, path, mtime);
...
static GdkPixbuf* theme_get_thumbnail_from_cache(Gde2ThemeMetaInfo* info, AppearanceData* data)
{
	...
	thumb_filename = gde2_desktop_thumbnail_factory_lookup(data->thumb_factory, path, mtime);
	g_free(path);
...
static void theme_thumbnail_generate(Gde2ThemeMetaInfo* info, AppearanceData* data)
{
...
static void theme_changed_on_disk_cb(Gde2ThemeCommonInfo* theme, Gde2ThemeChangeType change_type, Gde2ThemeElement element_type, AppearanceData* data)
{
	if (theme->type == GDE2_THEME_TYPE_METATHEME)
	{
		Gde2ThemeMetaInfo* meta = (Gde2ThemeMetaInfo*) theme;
...
static Gde2ThemeMetaInfo *
theme_load_from_gsettings (AppearanceData *data)
{
  Gde2ThemeMetaInfo *theme;
  gchar *scheme;

  theme = gde2_theme_meta_info_new ();
...
static gboolean
theme_is_equal (const Gde2ThemeMetaInfo *a, const Gde2ThemeMetaInfo *b)
{
  ...
  if ((a_set != b_set) ||
      (a_set && !gde2_theme_color_scheme_equal (a->gtk_color_scheme, b->gtk_color_scheme)))
    return FALSE;
...
static void
theme_set_custom_from_theme (const Gde2ThemeMetaInfo *info, AppearanceData *data)
{
  Gde2ThemeMetaInfo *custom = data->theme_custom;
...
theme_message_area_update (AppearanceData *data)
{
  ...
  show_error = !gde2_theme_meta_info_validate (theme, &error);
  ...
  if (show_error
      && g_error_matches (error, GDE2_THEME_ERROR, GDE2_THEME_ERROR_GTK_ENGINE_NOT_AVAILABLE)
...
{
  Gde2ThemeMetaInfo *gsettings_theme;
  const Gde2ThemeMetaInfo *selected;
  ...
  if (!selected || !(done = theme_is_equal (selected, gsettings_theme))) {
    ...
    theme_list = gde2_theme_meta_info_find_all ();

    for (l = theme_list; l; l = l->next) {
      Gde2ThemeMetaInfo *info = l->data;
  ...
  gde2_theme_meta_info_free (gsettings_theme);
}
...
static gint
theme_list_sort_func (Gde2ThemeMetaInfo *a,
                      Gde2ThemeMetaInfo *b)
{
...
void themes_init(AppearanceData* data)
{
  ...
  Gde2ThemeMetaInfo *meta_theme = NULL;
  GtkIconView *icon_view;
  ...
  data->theme_custom = gde2_theme_meta_info_new ();
  data->theme_icon = gdk_pixbuf_new_from_file (GDE2CC_PIXMAP_DIR "/theme-thumbnailing.png", NULL);
  data->theme_store = theme_store =
      gtk_list_store_new (NUM_COLS, GDK_TYPE_PIXBUF, G_TYPE_STRING, G_TYPE_STRING);

  /* set up theme list */
  theme_list = gde2_theme_meta_info_find_all ();
  gde2_theme_info_register_theme_change ((ThemeChangedCallback) theme_changed_on_disk_cb, data);
...
{
  gde2_theme_meta_info_free (data->theme_custom);
