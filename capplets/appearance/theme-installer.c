...
static gboolean
gde2_theme_install_real (GtkWindow *parent,
			  gint filetype,
			  const gchar *tmp_dir,
			...
			if (theme_type == THEME_GTK
				...
				if (gtk_dialog_run (GTK_DIALOG (dialog)) == GTK_RESPONSE_APPLY) {
					...
					case THEME_GTK:
						...
						break;
					case THEME_FINESTRA:
						settings = g_settings_new (FINESTRA_SCHEMA);
						g_settings_set_string (settings, FINESTRA_THEME_KEY, theme_name);
						g_object_unref (settings);
						break;
					case THEME_ICON:
						settings = g_settings_new (INTERFACE_SCHEMA);
				...
				dialog = gtk_message_dialog_new (parent,
								 ...
								 GTK_MESSAGE_INFO,
								 GTK_BUTTONS_OK,
								 _("GDE2 Theme %s correctly installed"),
								 theme_name);
				gtk_dialog_run (GTK_DIALOG (dialog));
			}
			gtk_widget_destroy (dialog);
		     ...
		     const char *path)
{
	...
	if (filetype == DIRECTORY) {
		...
		gde2_theme_install_real (parent,
					  filetype,
					  path,
					  name,
					  TRUE);
		...
		for (name = g_dir_read_name (dir); name && ok;
			...
			if (g_file_test (theme_dir, G_FILE_TEST_IS_DIR))
				ok = gde2_theme_install_real (parent,
							       filetype,
							       theme_dir,
							       name,
							       n_themes == 1);
		     ...
		     GtkWindow *parent)
{
	...
	if (g_str_has_suffix (base, ".tar.gz")
		...
		template = "gde2-theme-%d.gtp";
	else if (g_str_has_suffix (base, ".tar.bz2"))
		template = "gde2-theme-%d.tar.bz2";
	else {
...
void
gde2_theme_installer_run (GtkWindow *parent,
			   const gchar *filename)
{
	...
	gtk_file_filter_add_mime_type (filter, "application/x-gde2-theme-package");
	...
	if (gtk_dialog_run (GTK_DIALOG (dialog)) == GTK_RESPONSE_ACCEPT) {
		...
		if (uri_selected != NULL) {
			...
			gde2_theme_install (file, parent);
			g_object_unref (file);
		}
	} else {
		gtk_widget_destroy (dialog);
	...
	/*
	 * we're relying on the gde2 theme info module to pick up changes
