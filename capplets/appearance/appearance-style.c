...
update_color_buttons_from_string (const gchar *color_scheme, AppearanceData *data)
{
  ...
  if (!gde2_theme_color_scheme_parse (color_scheme, colors))
    return;
...
color_button_clicked_cb (GtkWidget *colorbutton, AppearanceData *data)
{
  ...
  if (!gde2_theme_color_scheme_equal (old_scheme, scheme->str)) {
...
gtk_theme_changed (GSettings *settings, gchar *key, AppearanceData *data)
{
  ...
  if (name) {
    ...
    theme = gde2_theme_info_find (name);
...
{
  Gde2ThemeIconInfo *theme = NULL;
  gchar *name;

  name = g_settings_get_string (settings, key);
  if (name) {
    theme = gde2_theme_icon_info_find (name);
    g_free (name);
}
...
cursor_size_scale_value_changed_cb (GtkRange *range, AppearanceData *data)
{
  ...
  theme = gde2_theme_cursor_info_find (name);
  g_free (name);
...
static void
update_cursor_size_scale (Gde2ThemeCursorInfo *theme,
                          AppearanceData *data)
{
#ifdef HAVE_XCURSOR
...
{
  Gde2ThemeCursorInfo *theme = NULL;
  gchar *name;
...
create_thumbnail (const gchar *name, GdkPixbuf *default_thumb, AppearanceData *data)
{
  if (default_thumb == data->icon_theme_icon) {
    Gde2ThemeIconInfo *info;
    info = gde2_theme_icon_info_find (name);
    if (info != NULL) {
...
static void
changed_on_disk_cb (Gde2ThemeCommonInfo *theme,
		    Gde2ThemeChangeType  change_type,
                    Gde2ThemeElement     element_type,
		    AppearanceData       *data)
{
  if (theme->type == GDE2_THEME_TYPE_REGULAR) {
    Gde2ThemeInfo *info = (Gde2ThemeInfo *) theme;
    ...
    Gde2ThemeIconInfo *info = (Gde2ThemeIconInfo *) theme;
    ...
    Gde2ThemeCursorInfo *info = (Gde2ThemeCursorInfo *) theme;
...
prepare_list (AppearanceData *data, GtkWidget *list, ThemeType type, GCallback callback)
{
  ...
  {
    Gde2ThemeCommonInfo *theme = (Gde2ThemeCommonInfo *) l->data;
    GtkTreeIter i;

    if (type == THEME_TYPE_CURSOR) {
      thumbnail = ((Gde2ThemeCursorInfo *) theme)->thumbnail;
    } else {
...
style_init (AppearanceData *data)
{
  ...
  data->gtk_theme_icon = gdk_pixbuf_new_from_file (GDE2CC_PIXMAP_DIR "/gtk-theme-thumbnailing.png", NULL);
  data->window_theme_icon = gdk_pixbuf_new_from_file (GDE2CC_PIXMAP_DIR "/window-theme-thumbnailing.png", NULL);
  data->icon_theme_icon = gdk_pixbuf_new_from_file (GDE2CC_PIXMAP_DIR "/icon-theme-thumbnailing.png", NULL);
  ...
  update_message_area (data);
  gde2_theme_info_register_theme_change ((ThemeChangedCallback) changed_on_disk_cb, data);
}
