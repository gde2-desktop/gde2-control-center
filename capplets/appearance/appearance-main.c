...
init_appearance_data (int *argc, char ***argv, GOptionContext *context)
{
  ...
  /* set up the data */
  uifile = g_build_filename (GDE2CC_GTKBUILDER_DIR, "appearance.ui",
                             NULL);
  ui = gtk_builder_new ();
  ...
  if (err)
  ...
  else
    {
      ...
      if (gde2_gsettings_schema_exists (CAJA_SCHEMA))
      ...
      data->finestra_settings = g_settings_new (FINESTRA_SCHEMA);
      data->mouse_settings = g_settings_new (MOUSE_SCHEMA);
      data->font_settings = g_settings_new (FONT_RENDER_SCHEMA);
      data->ui = ui;
      data->thumb_factory = gde2_desktop_thumbnail_factory_new (GDE2_DESKTOP_THUMBNAIL_SIZE_NORMAL);
    }
                      ...
                      AppearanceData *data)
{
      ...
      response_id == GTK_RESPONSE_DELETE_EVENT)
  {
    ...
    g_object_unref (data->finestra_settings);
...
main (int argc, char **argv)
{
  ...
  if (install_filename != NULL) {
    ...
    gde2_theme_install (inst, GTK_WINDOW (w));
    g_object_unref (inst);
  }
