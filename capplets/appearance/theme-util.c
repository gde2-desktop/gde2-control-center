...
{
  Gde2ThemeCommonInfo *info = theme;
  GFile *file;
  GFileInfo *file_info;
  gboolean writable;
...
gboolean theme_delete (const gchar *name, ThemeType type)
{
  ...
  switch (type) {
    ...
    case THEME_TYPE_ICON:
      theme = (Gde2ThemeCommonInfo *) gde2_theme_icon_info_find (name);
    ...
    case THEME_TYPE_META:
      theme = (Gde2ThemeCommonInfo *) gde2_theme_meta_info_find (name);
      theme_dir = g_strdup (theme->path);
      break;

    case THEME_TYPE_CURSOR:
      theme = (Gde2ThemeCommonInfo *) gde2_theme_cursor_info_find (name);
