...
Gde2BGPlacement wp_item_string_to_option (const gchar *option)
{
  if (!g_strcmp0(option, "centered"))
    return GDE2_BG_PLACEMENT_CENTERED;
  else if (!g_strcmp0(option, "stretched"))
    return GDE2_BG_PLACEMENT_FILL_SCREEN;
  else if (!g_strcmp0(option, "scaled"))
    return GDE2_BG_PLACEMENT_SCALED;
  else if (!g_strcmp0(option, "zoom"))
    return GDE2_BG_PLACEMENT_ZOOMED;
  else if (!g_strcmp0(option, "wallpaper"))
    return GDE2_BG_PLACEMENT_TILED;
  else if (!g_strcmp0(option, "spanned"))
    return GDE2_BG_PLACEMENT_SPANNED;
  else
    return GDE2_BG_PLACEMENT_SCALED;
}
...
Gde2BGColorType wp_item_string_to_shading (const gchar *shade_type)
{
  if (!g_strcmp0(shade_type, "solid"))
    return GDE2_BG_COLOR_SOLID;
  else if (!g_strcmp0(shade_type, "horizontal-gradient"))
    return GDE2_BG_COLOR_H_GRADIENT;
  else if (!g_strcmp0(shade_type, "vertical-gradient"))
    return GDE2_BG_COLOR_V_GRADIENT;
  else
    return GDE2_BG_COLOR_SOLID;
}

static void set_bg_properties (Gde2WPItem *item)
{
  if (item->filename)
    gde2_bg_set_filename (item->bg, item->filename);

  gde2_bg_set_color (item->bg, item->shade_type, item->pcolor, item->scolor);
  gde2_bg_set_placement (item->bg, item->options);
}
...
Gde2WPItem * gde2_wp_item_new (const gchar * filename,
  ...
  Gde2WPItem *item = g_new0 (Gde2WPItem, 1);

  item->filename = g_strdup (filename);
  item->fileinfo = gde2_wp_info_new (filename, thumbnails);
  ...
  if (item->fileinfo != NULL && item->fileinfo->mime_type != NULL &&
    ...
    gde2_wp_item_update (item);
    gde2_wp_item_ensure_gde2_bg (item);
    gde2_wp_item_update_description (item);
...
void gde2_wp_item_free (Gde2WPItem * item) {
  ...
  gde2_wp_info_free (item->fileinfo);
  if (item->bg)
...
GdkPixbuf * gde2_wp_item_get_frame_thumbnail (Gde2WPItem * item,
  ...
  if (frame != -1)
    pixbuf = gde2_bg_create_frame_thumbnail (item->bg, thumbs, gdk_screen_get_default (), width, height, frame);
  else
    pixbuf = gde2_bg_create_thumbnail (item->bg, thumbs, gdk_screen_get_default(), width, height);

  if (pixbuf && gde2_bg_changes_with_time (item->bg))
    {
      GdkPixbuf *tmp;
  ...
  gde2_bg_get_image_size (item->bg, thumbs, width, height, &item->width, &item->height);
...
GdkPixbuf * gde2_wp_item_get_thumbnail (Gde2WPItem * item,
					 Gde2DesktopThumbnailFactory * thumbs,
                                         gint width,
                                         gint height) {
  return gde2_wp_item_get_frame_thumbnail (item, thumbs, width, height, -1);
}
...
void gde2_wp_item_update_description (Gde2WPItem * item) {
  ...
  if (!strcmp (item->filename, "(none)")) {
    ...
    if (strcmp (item->fileinfo->mime_type, "application/xml") == 0)
      {
        if (gde2_bg_changes_with_time (item->bg))
    ...
    if (gde2_bg_has_multiple_sizes (item->bg))
