...
#include "gde2-wp-item.h"
#include <gio/gio.h>
#include <string.h>
#include <libxml/parser.h>

static gboolean gde2_wp_xml_get_bool(const xmlNode* parent, const char* prop_name)
{
...
static void gde2_wp_xml_set_bool(const xmlNode* parent, const xmlChar* prop_name, gboolean value)
{
	if (parent != NULL && prop_name != NULL)
	{
		if (value)
...
static void gde2_wp_load_legacy(AppearanceData* data)
{
	/* Legacy of GNOME2
	...
	if (g_file_test(filename, G_FILE_TEST_EXISTS))
	{
		...
		if ((fp = fopen(filename, "r")) != NULL)
		{
			...
			while (fgets(foo, 4096, fp))
			{
				...
				item = gde2_wp_item_new(foo, data->wp_hash, data->thumb_factory);

				if (item != NULL && item->fileinfo == NULL)
				{
					gde2_wp_item_free(item);
				}
			}
...
static void gde2_wp_xml_load_xml(AppearanceData* data, const char* filename)
{
	xmlDoc* wplist;
	xmlNode* root;
	xmlNode* list;
	...
	for (list = root->children; list != NULL; list = list->next)
	{
		if (!strcmp((char*) list->name, "wallpaper"))
		{
			...
			wp = g_new0(Gde2WPItem, 1);

			wp->deleted = gde2_wp_xml_get_bool(list, "deleted");
			...
			{
				wp->fileinfo = gde2_wp_info_new(wp->filename, data->thumb_factory);
				...
				gde2_wp_item_ensure_gde2_bg (wp);
				gde2_wp_item_update_description (wp);
...
static void gde2_wp_file_changed(GFileMonitor* monitor, GFile* file, GFile* other_file, GFileMonitorEvent event_type, AppearanceData* data)
{
	char* filename;
	...
	switch (event_type)
	{
		...
		case G_FILE_MONITOR_EVENT_CREATED:
			...
			gde2_wp_xml_load_xml(data, filename);
			g_free(filename);
			break;
		default:
			break;
...
static void gde2_wp_xml_add_monitor(GFile* directory, AppearanceData* data)
{
	GError* error = NULL;
	...
	g_signal_connect(monitor, "changed", G_CALLBACK(gde2_wp_file_changed), data);
}

static void gde2_wp_xml_load_from_dir(const char* path, AppearanceData* data)
{
	GFile* directory;
	...
	while ((info = g_file_enumerator_next_file(enumerator, NULL, NULL)))
	{
		...
		gde2_wp_xml_load_xml(data, fullpath);
		g_free(fullpath);
	}

	g_file_enumerator_close(enumerator, NULL, NULL);

	gde2_wp_xml_add_monitor(directory, data);

	g_object_unref(directory);
}

void gde2_wp_xml_load_list(AppearanceData* data)
{
	...
	gint i;
		...
		wpdbfile = g_build_filename(g_get_user_config_dir(), "gde2", "backgrounds.xml", NULL);

	if (g_file_test(wpdbfile, G_FILE_TEST_EXISTS))
	{
		gde2_wp_xml_load_xml(data, wpdbfile);
	}
	else
	{
		g_free (wpdbfile);

			wpdbfile = g_build_filename(g_get_user_config_dir(), "gde2", "wp-list.xml", NULL);
	...
	datadir = g_build_filename(g_get_user_data_dir(), "gde2-background-properties", NULL);
	gde2_wp_xml_load_from_dir(datadir, data);
	g_free(datadir);
	...
	{
		datadir = g_build_filename(system_data_dirs[i], "gde2-background-properties", NULL);
		gde2_wp_xml_load_from_dir(datadir, data);
		g_free (datadir);
	}

	gde2_wp_xml_load_from_dir(WALLPAPER_DATADIR, data);

	gde2_wp_load_legacy(data);
}

static void gde2_wp_list_flatten(const char* key, Gde2WPItem* item, GSList** list)
{
...
void gde2_wp_xml_save_list(AppearanceData* data)
{
	xmlDoc* wplist;
	xmlNode* root;
	xmlNode* wallpaper;
	...
	g_hash_table_foreach(data->wp_hash, (GHFunc) gde2_wp_list_flatten, &list);
	g_hash_table_destroy(data->wp_hash);
	list = g_slist_reverse(list);

		wpfile = g_build_filename(g_get_user_config_dir(), "gde2", "backgrounds.xml", NULL);

	xmlKeepBlanksDefault(0);

	wplist = xmlNewDoc((xmlChar*) "1.0");
	xmlCreateIntSubset(wplist, (xmlChar*) "wallpapers", NULL, (xmlChar*) "gde2-wp-list.dtd");
	root = xmlNewNode(NULL, (xmlChar*) "wallpapers");
	xmlDocSetRootElement(wplist, root);

	while (list != NULL)
	{
		Gde2WPItem* wpitem = list->data;
		const char* none = "(none)";
		char* filename;
		const char* scale;
		const char* shade;
		...
		gde2_wp_xml_set_bool(wallpaper, (xmlChar*) "deleted", wpitem->deleted);
