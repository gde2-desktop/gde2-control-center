...
/* taken from gde2-desktop-item.c */
static gchar *
...
static gboolean
write_theme_to_disk (Gde2ThemeMetaInfo  *theme_info,
		     ...
		     GError             **error)
{
  ...
  str = g_strdup_printf(theme_header, theme_name, theme_description, theme_info->gtk_theme_name, theme_info->finestra_theme_name, theme_info->icon_theme_name);
...
static gboolean
save_theme_to_disk (Gde2ThemeMetaInfo  *theme_info,
		    const gchar         *theme_name,
		    const gchar         *theme_description,
		    gboolean		 save_background,
		    gboolean             save_notification,
		      ...
		      AppearanceData *data)
{
  if (response_id == GTK_RESPONSE_OK) {
    ...
    g_free (buffer_text);
    theme_info = (Gde2ThemeMetaInfo *) g_object_get_data (G_OBJECT (save_dialog), "meta-theme-info");
...
void
theme_save_dialog_run (Gde2ThemeMetaInfo *theme_info,
		       AppearanceData     *data)
{
  GtkWidget *entry;
  GtkWidget *text_view;
  ...
  if (data->theme_save_dialog == NULL) {
    ...
    error_quark = g_quark_from_string ("gde2-theme-save");
