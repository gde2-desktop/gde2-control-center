...
static void select_item(AppearanceData* data, Gde2WPItem* item, gboolean scroll)
{
	GtkTreePath* path;
...
static Gde2WPItem* get_selected_item(AppearanceData* data, GtkTreeIter* iter)
{
	Gde2WPItem* item = NULL;
	GList* selected;
...
{
  Gde2BG *bg = data;
  Gde2WPItem *item = value;

  return item->bg == bg;
}

static void on_item_changed (Gde2BG *bg, AppearanceData *data) {
  GtkTreeModel *model;
  GtkTreeIter iter;
  GtkTreePath *path;
  Gde2WPItem *item;
  ...
  if (gtk_tree_model_get_iter (model, &iter, path)) {
    ...
    pixbuf = gde2_wp_item_get_thumbnail (item,
                                          data->thumb_factory,
                                          data->thumb_width,
                                          data->thumb_height);
    if (pixbuf) {
                         ...
                         AppearanceData *data)
{
  ...
  pixbuf = gde2_wp_item_get_thumbnail (item, data->thumb_factory,
                                        data->thumb_width,
                                        data->thumb_height);
              ...
              const gchar *filename)
{
  ...
  {
    item = gde2_wp_item_new (filename, data->wp_hash, data->thumb_factory);
...
static gboolean
wp_props_wp_set (AppearanceData *data, Gde2WPItem *item)
{
  gchar *pcolor, *scolor;
		  ...
		  AppearanceData *data)
{
  if (type == TARGET_URI_LIST) {
    Gde2WPItem *item = get_selected_item (data, NULL);
                   ...
                   AppearanceData *data)
{
  ...
  if (uri)
  {
    ...
    if (mime_type)
    {
      pixbuf = gde2_desktop_thumbnail_factory_generate_thumbnail (data->thumb_factory,
								   uri,
								   mime_type);
    }
...
wp_load_stuffs (void *user_data)
{
  ...
  if (item == NULL)
  {
    item = gde2_wp_item_new ("(none)", data->wp_hash, data->thumb_factory);
    if (item != NULL)
    {
      wp_props_load_wallpaper (item->filename, item, data);
    }
  ...
  else
  {
    ...
    {
      select_item (data, item, FALSE);
      wp_option_menu_set (data, GDE2_BG_PLACEMENT_SCALED, FALSE);
    }
  }
  g_free (imagepath);
  g_free (style);
            ...
            gint             direction)
{
  ...
  if (frame >= 0)
    pixbuf = gde2_wp_item_get_frame_thumbnail (item,
                                                data->thumb_factory,
                                                data->thumb_width,
                                                data->thumb_height,
                                                frame);
                        ...
                        gpointer         user_data)
{
  ...
  if (gtk_icon_view_path_is_selected (GTK_ICON_VIEW (layout), path)) {
    item = get_selected_item (data, NULL);
    visible = gde2_bg_changes_with_time (item->bg);
  }
  else
    visible = FALSE;
