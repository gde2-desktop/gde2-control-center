/* gde2-theme-info.c - GDE2 Theme information
 *
 ...
 * Gde2Theme{Meta,Icon,}Info struct.  Note that the uri is of the form
 ...
 * by them is a GList whose data elements are Gde2Theme{Meta,Icon,}Info
                           ...
                           gpointer    data)
{
  ...
  if (list == NULL) {
    ...
    while (list_ptr) {
      ...
      theme_priority = ((Gde2ThemeCommonInfo *) list_ptr->data)->priority;
                             ...
                             gint         priority)
{
  ...
  while (list) {
    Gde2ThemeCommonInfo *info = (Gde2ThemeCommonInfo *) list->data;
...
static gint
theme_compare (Gde2ThemeCommonInfo *a,
               Gde2ThemeCommonInfo *b)
{
  gint cmp;
  ...
  switch (a->type) {
  case GDE2_THEME_TYPE_METATHEME:
    cmp = gde2_theme_meta_info_compare (
                    (Gde2ThemeMetaInfo *) a, (Gde2ThemeMetaInfo *) b);
    break;
  case GDE2_THEME_TYPE_ICON:
    cmp = gde2_theme_icon_info_compare (
                    (Gde2ThemeIconInfo *) a, (Gde2ThemeIconInfo *) b);
    break;
  case GDE2_THEME_TYPE_CURSOR:
    cmp = gde2_theme_cursor_info_compare (
                    (Gde2ThemeCursorInfo *) a, (Gde2ThemeCursorInfo *) b);
    break;
  default:
...
static void
theme_free (Gde2ThemeCommonInfo *info)
{
  switch (info->type) {
  case GDE2_THEME_TYPE_METATHEME:
    gde2_theme_meta_info_free ((Gde2ThemeMetaInfo *) info);
    break;
  case GDE2_THEME_TYPE_ICON:
    gde2_theme_icon_info_free ((Gde2ThemeIconInfo *) info);
    break;
  case GDE2_THEME_TYPE_REGULAR:
    gde2_theme_info_free ((Gde2ThemeInfo *) info);
    break;
  case GDE2_THEME_TYPE_CURSOR:
    gde2_theme_cursor_info_free ((Gde2ThemeCursorInfo *) info);
    break;
  default:
    g_assert_not_reached ();
  }
...
{
	return g_quark_from_static_string("gde2-theme-info-error-quark");
}
...
Gde2ThemeMetaInfo* gde2_theme_read_meta_theme(GFile* meta_theme_uri)
{
	...
	Gde2DesktopItem* meta_theme_ditem;
	gchar* meta_theme_file;
	const gchar* str;
	gchar* scheme;

	meta_theme_file = g_file_get_uri(meta_theme_uri);
	meta_theme_ditem = gde2_desktop_item_new_from_uri(meta_theme_file, 0, NULL);
	...
	meta_theme_info = gde2_theme_meta_info_new();
	...
	str = gde2_desktop_item_get_localestring(meta_theme_ditem, THEME_NAME);

	if (!str)
	{
		str = gde2_desktop_item_get_localestring(meta_theme_ditem, GDE2_DESKTOP_ITEM_NAME);
		if (!str)
		{ /* shouldn't reach */
			gde2_theme_meta_info_free(meta_theme_info);
			return NULL;
		}
	}
	...
	str = gde2_desktop_item_get_localestring(meta_theme_ditem, THEME_COMMENT);

	if (str == NULL)
		str = gde2_desktop_item_get_localestring(meta_theme_ditem, GDE2_DESKTOP_ITEM_COMMENT);

	if (str != NULL)
		meta_theme_info->comment = g_strdup(str);

	str = gde2_desktop_item_get_string(meta_theme_ditem, GDE2_DESKTOP_ITEM_ICON);

	if (str != NULL)
		meta_theme_info->icon_file = g_strdup(str);

	str = gde2_desktop_item_get_string(meta_theme_ditem, GTK_THEME_KEY);
	...
	str = gde2_desktop_item_get_string(meta_theme_ditem, GTK_COLOR_SCHEME_KEY);
	...
	str = gde2_desktop_item_get_string(meta_theme_ditem, NOTIFICATION_THEME_KEY);

	if (str != NULL)
		meta_theme_info->notification_theme_name = g_strdup(str);

	str = gde2_desktop_item_get_string(meta_theme_ditem, CURSOR_THEME_KEY);

	if (str != NULL)
	{
		meta_theme_info->cursor_theme_name = g_strdup(str);

		str = gde2_desktop_item_get_string(meta_theme_ditem, CURSOR_SIZE_KEY);
	...
	str = gde2_desktop_item_get_string(meta_theme_ditem, APPLICATION_FONT_KEY);

	if (str != NULL)
		meta_theme_info->application_font = g_strdup(str);

	str = gde2_desktop_item_get_string(meta_theme_ditem, DOCUMENTS_FONT_KEY);

	if (str != NULL)
		meta_theme_info->documents_font = g_strdup(str);

	str = gde2_desktop_item_get_string(meta_theme_ditem, DESKTOP_FONT_KEY);

	if (str != NULL)
		meta_theme_info->desktop_font = g_strdup(str);

	str = gde2_desktop_item_get_string(meta_theme_ditem, WINDOWTITLE_FONT_KEY);

	if (str != NULL)
		meta_theme_info->windowtitle_font = g_strdup(str);

	str = gde2_desktop_item_get_string(meta_theme_ditem, MONOSPACE_FONT_KEY);

	if (str != NULL)
		meta_theme_info->monospace_font = g_strdup(str);

	str = gde2_desktop_item_get_string(meta_theme_ditem, BACKGROUND_IMAGE_KEY);

	if (str != NULL)
		meta_theme_info->background_image = g_strdup(str);

	meta_theme_info->hidden = gde2_desktop_item_get_boolean(meta_theme_ditem, HIDDEN_KEY);

	gde2_desktop_item_unref(meta_theme_ditem);

	return meta_theme_info;
}

static Gde2ThemeIconInfo *
read_icon_theme (GFile *icon_theme_uri)
{
  Gde2ThemeIconInfo *icon_theme_info;
  Gde2DesktopItem *icon_theme_ditem;
  ...
  icon_theme_ditem = gde2_desktop_item_new_from_uri (icon_theme_file, 0, NULL);
  g_free (icon_theme_file);

  if (icon_theme_ditem == NULL)
    return NULL;

  name = gde2_desktop_item_get_localestring (icon_theme_ditem, "Icon Theme/Name");
  if (!name) {
    name = gde2_desktop_item_get_localestring (icon_theme_ditem, GDE2_DESKTOP_ITEM_NAME);
    if (!name) {
      gde2_desktop_item_unref (icon_theme_ditem);
      return NULL;
    }
  }

  /* If index.theme has no Directories entry, it is only a cursor theme */
  directories = gde2_desktop_item_get_string (icon_theme_ditem, "Icon Theme/Directories");
  if (directories == NULL) {
    gde2_desktop_item_unref (icon_theme_ditem);
    return NULL;
  }

  icon_theme_info = gde2_theme_icon_info_new ();
  icon_theme_info->readable_name = g_strdup (name);
  icon_theme_info->path = g_file_get_path (icon_theme_uri);
  icon_theme_info->hidden = gde2_desktop_item_get_boolean (icon_theme_ditem, "Icon Theme/Hidden");
...
{
  Gde2ThemeCursorInfo *theme_info;

  theme_info = gde2_theme_cursor_info_new ();
...
static Gde2ThemeCursorInfo *
read_cursor_theme (GFile *cursor_theme_uri)
{
  Gde2ThemeCursorInfo *cursor_theme_info = NULL;
  ...
  if (get_file_type (cursors_uri) == G_FILE_TYPE_DIRECTORY) {
    ...
    if (sizes->len == 0) {
      ...
      Gde2DesktopItem *cursor_theme_ditem;
      ...
      cursor_theme_info = gde2_theme_cursor_info_new ();
      ...
      cursor_theme_ditem = gde2_desktop_item_new_from_file (cursor_theme_file, 0, NULL);
      g_free (cursor_theme_file);

      if (cursor_theme_ditem != NULL) {
        const gchar *readable_name;

        readable_name = gde2_desktop_item_get_string (cursor_theme_ditem,
                                                       "Icon Theme/Name");
        if (readable_name)
        ...
        cursor_theme_info->hidden = gde2_desktop_item_get_boolean (cursor_theme_ditem,
                                                                    "Icon Theme/Hidden");

        gde2_desktop_item_unref (cursor_theme_ditem);
      } else {
        cursor_theme_info->readable_name = g_strdup (name);
      }
    }
...
read_current_cursor_font (void)
{
  ...
  struct dirent *file_dirent;
	...
	dir_name = g_build_filename(g_get_user_config_dir(), "gde2", "share", "cursor-fonts", NULL);
...
read_cursor_fonts (void)
{
  ...
  const gchar *builtins[][4] = {
    {
      "gde2/cursor-fonts/cursor-normal.pcf",
      N_("Default Pointer"),
      N_("Default Pointer - Current"),
      "mouse-cursor-normal.png"
    }, {
      "gde2/cursor-fonts/cursor-white.pcf",
      N_("White Pointer"),
      N_("White Pointer - Current"),
      "mouse-cursor-white.png"
    }, {
      "gde2/cursor-fonts/cursor-large.pcf",
      N_("Large Pointer"),
      N_("Large Pointer - Current"),
      "mouse-cursor-normal-large.png"
    }, {
      "gde2/cursor-fonts/cursor-large-white.pcf",
  ...
  for (i = 0; i < G_N_ELEMENTS (builtins); i++) {
    ...
    filename = g_build_filename (GDE2CC_DATA_DIR, "pixmaps", builtins[i][3], NULL);
    theme_info->thumbnail = gdk_pixbuf_new_from_file (filename, NULL);
    g_free (filename);

    theme_info->path = g_build_filename (GDE2CC_DATA_DIR, builtins[i][0], NULL);
...
handle_change_signal (gpointer             data,
                      Gde2ThemeChangeType change_type,
                      Gde2ThemeElement    element_type)
{
#ifdef DEBUG
  gchar *type_str = NULL;
...
#endif
  Gde2ThemeCommonInfo *theme = data;
  GList *list;
...
#ifdef DEBUG
  if (theme->type == GDE2_THEME_TYPE_METATHEME)
    type_str = "meta";
  else if (theme->type == GDE2_THEME_TYPE_ICON)
    type_str = "icon";
  else if (theme->type == GDE2_THEME_TYPE_CURSOR)
    type_str = "cursor";
  else if (theme->type == GDE2_THEME_TYPE_REGULAR) {
  ...
  if (change_type == GDE2_THEME_CHANGE_CREATED)
    ...
    change_str = "changed";
  else if (change_type == GDE2_THEME_CHANGE_DELETED)
    change_str = "deleted";

  if (type == GDE2_THEME_TYPE_REGULAR) {
                    ...
                    gint              priority)
{
  ...
  if (theme_info == NULL) {
    if (theme_exists) {
      theme_info = gde2_theme_info_new ();
      ...
      if (key_element & GDE2_THEME_GTK_2)
      ...
      handle_change_signal (theme_info, GDE2_THEME_CHANGE_CREATED, key_element);
    }
  } else {
    ...
    if (theme_exists && theme_used_to_exist) {
      handle_change_signal (theme_info, GDE2_THEME_CHANGE_CHANGED, key_element);
...
update_gtk2_index (GFile *gtk2_index_uri,
                   gint   priority)
{
  update_theme_index (gtk2_index_uri, GDE2_THEME_GTK_2, priority);
}
...
update_keybinding_index (GFile *keybinding_index_uri,
                         gint   priority)
{
  update_theme_index (keybinding_index_uri, GDE2_THEME_GTK_2_KEYBINDING, priority);
}
...
{
  gboolean theme_exists;
  Gde2ThemeCommonInfo *theme_info = NULL;
  Gde2ThemeCommonInfo *old_theme_info;
  GFile *common_theme_dir_uri;
  gchar *common_theme_dir;
  GHashTable *hash_by_uri;
  GHashTable *hash_by_name;

  if (type == GDE2_THEME_TYPE_ICON) {
    hash_by_uri = icon_theme_hash_by_uri;
    hash_by_name = icon_theme_hash_by_name;
  } else if (type == GDE2_THEME_TYPE_CURSOR) {
  ...
  if (type != GDE2_THEME_TYPE_CURSOR) {
    ...
    if (get_file_type (theme_index_uri) == G_FILE_TYPE_REGULAR) {
      ...
      if (type == GDE2_THEME_TYPE_ICON)
        theme_info = (Gde2ThemeCommonInfo *) read_icon_theme (theme_index_uri);
      else
        theme_info = (Gde2ThemeCommonInfo *) gde2_theme_read_meta_theme (theme_index_uri);
    } else {
      theme_info = NULL;
    }
  ...
  /* cursor themes don't necessarily have an index file, so try those in any case */
    ...
    theme_info = (Gde2ThemeCommonInfo *) read_cursor_theme (theme_index_uri);
  }
#endif
  ...
  old_theme_info = (Gde2ThemeCommonInfo *) g_hash_table_lookup (hash_by_uri, common_theme_dir);
  ...
  if (old_theme_info == NULL) {
    if (theme_exists) {
      ...
      handle_change_signal (theme_info, GDE2_THEME_CHANGE_CREATED, 0);
    }
  } else {
    if (theme_exists) {
      if (theme_compare (theme_info, old_theme_info) != 0) {
        ...
        g_hash_table_insert (hash_by_uri, g_strdup (common_theme_dir), theme_info);
        add_theme_to_hash_by_name (hash_by_name, theme_info);
        handle_change_signal (theme_info, GDE2_THEME_CHANGE_CHANGED, 0);
      ...
      handle_change_signal (old_theme_info, GDE2_THEME_CHANGE_DELETED, 0);
      theme_free (old_theme_info);
    }
  }
...
update_meta_theme_index (GFile *meta_theme_index_uri,
                         gint   priority)
{
  update_common_theme_dir_index (meta_theme_index_uri, GDE2_THEME_TYPE_METATHEME, priority);
}
...
update_icon_theme_index (GFile *icon_theme_index_uri,
                         gint   priority)
{
  update_common_theme_dir_index (icon_theme_index_uri, GDE2_THEME_TYPE_ICON, priority);
}
...
{
#ifdef HAVE_XCURSOR
  update_common_theme_dir_index (cursor_theme_index_uri, GDE2_THEME_TYPE_CURSOR, priority);
#endif
}
...
gde2_theme_info_new (void)
{
  ...
  theme_info = g_new0 (Gde2ThemeInfo, 1);
  theme_info->type = GDE2_THEME_TYPE_REGULAR;

  return theme_info;
}

void
gde2_theme_info_free (Gde2ThemeInfo *theme_info)
{
...
Gde2ThemeInfo *
gde2_theme_info_find (const gchar *theme_name)
{
  return (Gde2ThemeInfo *)
         get_theme_from_hash_by_name (theme_hash_by_name, theme_name, -1);
}

struct Gde2ThemeInfoHashData
{
...
static void
gde2_theme_info_find_by_type_helper (gpointer key,
                                      GList *list,
                                      struct Gde2ThemeInfoHashData *hash_data)
{
...
GList *
gde2_theme_info_find_by_type (guint elements)
{
  struct Gde2ThemeInfoHashData data;
  data.user_data = GINT_TO_POINTER (elements);
  data.list = NULL;

  g_hash_table_foreach (theme_hash_by_name,
                        (GHFunc) gde2_theme_info_find_by_type_helper,
                        &data);

  return data.list;
}

static void gde2_theme_info_find_all_helper(const gchar* key, GList* list, GList** themes)
{
	/* only return visible themes */
	if (!((Gde2ThemeCommonInfo*) list->data)->hidden)
	{
...
/* Icon themes */
Gde2ThemeIconInfo *
gde2_theme_icon_info_new (void)
{
  Gde2ThemeIconInfo *icon_theme_info;

  icon_theme_info = g_new0 (Gde2ThemeIconInfo, 1);
  icon_theme_info->type = GDE2_THEME_TYPE_ICON;

  return icon_theme_info;
}

void
gde2_theme_icon_info_free (Gde2ThemeIconInfo *icon_theme_info)
{
...
gint
gde2_theme_icon_info_compare (Gde2ThemeIconInfo *a,
                               Gde2ThemeIconInfo *b)
{
  gint cmp;
...
/* Cursor themes */
Gde2ThemeCursorInfo *
gde2_theme_cursor_info_new (void)
{
  Gde2ThemeCursorInfo *theme_info;

  theme_info = g_new0 (Gde2ThemeCursorInfo, 1);
  theme_info->type = GDE2_THEME_TYPE_CURSOR;

  return theme_info;
}

void
gde2_theme_cursor_info_free (Gde2ThemeCursorInfo *cursor_theme_info)
{
...
gint
gde2_theme_cursor_info_compare (Gde2ThemeCursorInfo *a,
...
{
	Gde2ThemeMetaInfo* theme_info;

	theme_info = g_new0(Gde2ThemeMetaInfo, 1);
	theme_info->type = GDE2_THEME_TYPE_METATHEME;
...
gboolean gde2_theme_meta_info_validate(const Gde2ThemeMetaInfo* info, GError** error)
{
	Gde2ThemeInfo* theme;

	g_return_val_if_fail (error == NULL || *error == NULL, FALSE);

	theme = gde2_theme_info_find (info->gtk_theme_name);

	if (!theme || !theme->has_gtk)
	{
		g_set_error (error, GDE2_THEME_ERROR, GDE2_THEME_ERROR_GTK_THEME_NOT_AVAILABLE,
	...
	if (!gde2_theme_icon_info_find (info->icon_theme_name))
	{
		g_set_error (error, GDE2_THEME_ERROR, GDE2_THEME_ERROR_ICON_THEME_NOT_AVAILABLE,
	...
	if (engine != NULL)
	{
		g_set_error (error, GDE2_THEME_ERROR, GDE2_THEME_ERROR_GTK_ENGINE_NOT_AVAILABLE,
...
Gde2ThemeMetaInfo* gde2_theme_meta_info_find(const char* meta_theme_name)
{
	g_return_val_if_fail(meta_theme_name != NULL, NULL);

	return (Gde2ThemeMetaInfo*) get_theme_from_hash_by_name (meta_theme_hash_by_name, meta_theme_name, -1);
}
...
GList* gde2_theme_meta_info_find_all(void)
{
  ...
  g_hash_table_foreach (meta_theme_hash_by_name, (GHFunc) gde2_theme_info_find_all_helper, &list);

  return list;
}

gint
gde2_theme_meta_info_compare (Gde2ThemeMetaInfo *a,
                               Gde2ThemeMetaInfo *b)
{
  gint cmp;
...
void
gde2_theme_info_register_theme_change (ThemeChangedCallback func,
                                        gpointer data)
{
...
gboolean
#if GTK_CHECK_VERSION (3, 0, 0)
gde2_theme_color_scheme_parse (const gchar *scheme, GdkRGBA *colors)
#else
gde2_theme_color_scheme_parse (const gchar *scheme, GdkColor *colors)
#endif
{
...
gboolean
gde2_theme_color_scheme_equal (const gchar *s1, const gchar *s2)
{
#if GTK_CHECK_VERSION (3, 0, 0)
  ...
  if (!gde2_theme_color_scheme_parse (s1, c1) ||
      !gde2_theme_color_scheme_parse (s2, c2))
    return FALSE;
...
gde2_theme_init ()
{
  ...
  if (!gde2_theme_cursor_info_find ("default"))
    add_default_cursor_theme ();
#else
