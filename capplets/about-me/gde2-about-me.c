...
#include "gde2-about-me-password.h"
#include "gde2-about-me-fingerprint.h"
#include "marshal.h"
...
static void
about_me_load_photo (Gde2AboutMe *me)
{
	gchar         *file;
	GError        *error = NULL;
...
static void
about_me_update_photo (Gde2AboutMe *me)
{
	gchar         *file;
	GError        *error;
...
static void
about_me_load_info (Gde2AboutMe *me)
{
			 ...
			 Gde2AboutMe   *me)
{
	...
	if (uri) {
		...
		if (!me->thumbs)
			me->thumbs = gde2_desktop_thumbnail_factory_new (GDE2_DESKTOP_THUMBNAIL_SIZE_NORMAL);
		...
		if (file_info != NULL) {
			...
			if (content_type) {
				...
				pixbuf = gde2_desktop_thumbnail_factory_generate_thumbnail (me->thumbs,
										     uri,
										     mime_type);
				g_free (mime_type);
			}
...
static void
about_me_image_clicked_cb (GtkWidget *button, Gde2AboutMe *me)
{
...
static void
about_me_image_changed_cb (GtkWidget *widget, Gde2AboutMe *me)
{
	me->have_image = TRUE;
	me->image_changed = TRUE;
	about_me_update_photo (me);
...
static void
about_me_button_clicked_cb (GtkDialog *dialog, gint response_id, Gde2AboutMe *me)
{
...
static void
about_me_passwd_clicked_cb (GtkWidget *button, Gde2AboutMe *me)
{
	GtkBuilder *dialog;

	dialog = me->dialog;
	gde2_about_me_password (GTK_WINDOW (WID ("about-me-dialog")));
}

static void
about_me_fingerprint_button_clicked_cb (GtkWidget *button, Gde2AboutMe *me)
{
...
about_me_setup_dialog (void)
{
	...
	me = g_new0 (Gde2AboutMe, 1);
	me->image = NULL;

	dialog = gtk_builder_new ();
	gtk_builder_add_from_file (dialog, GDE2CC_UI_DIR "/gde2-about-me-dialog.ui", NULL);
