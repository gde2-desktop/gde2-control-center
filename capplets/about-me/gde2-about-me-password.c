 ...
 * 02110-1301, USA.
 *
 * Parts of this code come from Gde2-System-Tools.
 */
 ...
 * Error handling {{
 */
#define PASSDLG_ERROR (gde2_about_me_password_error_quark())

GQuark gde2_about_me_password_error_quark(void)
{
	static GQuark q = 0;

	if (q == 0) {
		q = g_quark_from_static_string("gde2_about_me_password_error");
	}
...
passdlg_init (PasswordDialog *pdialog, GtkWindow *parent)
{
    ...
    gtk_builder_add_from_file (dialog, GDE2CC_UI_DIR "/gde2-about-me-password.ui", NULL);
	pdialog->ui = dialog;
...
/* Main */
void
gde2_about_me_password (GtkWindow *parent)
{
