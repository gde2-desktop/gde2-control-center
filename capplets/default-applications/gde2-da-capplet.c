...
static void
set_changed(GtkComboBox* combo, Gde2DACapplet* capplet, GList* list, gint type)
{
	guint index;
	GAppInfo* item;
...
static void
close_cb(GtkWidget* window, gint response, Gde2DACapplet* capplet)
{
...
static void
web_combo_changed_cb(GtkComboBox* combo, Gde2DACapplet* capplet)
{
	set_changed(combo, capplet, capplet->web_browsers, DA_TYPE_WEB_BROWSER);
}

static void
mail_combo_changed_cb(GtkComboBox* combo, Gde2DACapplet* capplet)
{
	set_changed(combo, capplet, capplet->mail_readers, DA_TYPE_EMAIL);
}

static void
file_combo_changed_cb(GtkComboBox* combo, Gde2DACapplet* capplet)
{
	set_changed(combo, capplet, capplet->file_managers, DA_TYPE_FILE);
}

static void
text_combo_changed_cb(GtkComboBox* combo, Gde2DACapplet* capplet)
{
	set_changed(combo, capplet, capplet->text_editors, DA_TYPE_TEXT);
}

static void
media_combo_changed_cb(GtkComboBox* combo, Gde2DACapplet* capplet)
{
	set_changed(combo, capplet, capplet->media_players, DA_TYPE_MEDIA);
}

static void
video_combo_changed_cb(GtkComboBox* combo, Gde2DACapplet* capplet)
{
	set_changed(combo, capplet, capplet->video_players, DA_TYPE_VIDEO);
}

static void
terminal_combo_changed_cb(GtkComboBox* combo, Gde2DACapplet* capplet)
{
	set_changed(combo, capplet, capplet->terminals, DA_TYPE_TERMINAL);
}

static void
visual_combo_changed_cb(GtkComboBox* combo, Gde2DACapplet* capplet)
{
	set_changed(combo, capplet, capplet->visual_ats, DA_TYPE_VISUAL);
}

static void
mobility_combo_changed_cb(GtkComboBox* combo, Gde2DACapplet* capplet)
{
	set_changed(combo, capplet, capplet->mobility_ats, DA_TYPE_MOBILITY);
}

static void
image_combo_changed_cb(GtkComboBox* combo, Gde2DACapplet* capplet)
{
	set_changed(combo, capplet, capplet->image_viewers, DA_TYPE_IMAGE);
}

static void
document_combo_changed_cb(GtkComboBox* combo, Gde2DACapplet* capplet)
{
	set_changed(combo, capplet, capplet->document_viewers, DA_TYPE_DOCUMENT);
}

static void
word_combo_changed_cb(GtkComboBox* combo, Gde2DACapplet* capplet)
{
	set_changed(combo, capplet, capplet->word_editors, DA_TYPE_WORD);
}

static void
spreadsheet_combo_changed_cb(GtkComboBox* combo, Gde2DACapplet* capplet)
{
...
static void
theme_changed_cb(GtkIconTheme* theme, Gde2DACapplet* capplet)
{
	GObject* icon;
	gint i;
...
static void
screen_changed_cb(GtkWidget* widget, GdkScreen* screen, Gde2DACapplet* capplet)
{
	GtkIconTheme* theme;
...
static void
show_dialog(Gde2DACapplet* capplet, const gchar* start_page)
{
	...
	if (g_file_test(GDE2CC_UI_DIR "/gde2-default-applications-properties.ui", G_FILE_TEST_EXISTS) != FALSE)
	{
		builder_result = gtk_builder_add_from_file(builder, GDE2CC_UI_DIR "/gde2-default-applications-properties.ui", NULL);
	}
	else
	{
		builder_result = gtk_builder_add_from_file(builder, "./gde2-default-applications-properties.ui", NULL);
	}
...
main(int argc, char** argv)
{
	...
	GOptionContext* context = g_option_context_new(_("- GDE2 Default Applications"));
	g_option_context_add_main_entries(context, option_entries, GETTEXT_PACKAGE);

	capplet_init(context, &argc, &argv);

	Gde2DACapplet* capplet = g_new0(Gde2DACapplet, 1);
