...
{
    Gde2RRScreen       *screen;
    Gde2RRConfig  *current_configuration;
    Gde2RRLabeler *labeler;
    Gde2RROutputInfo         *current_output;
...
static gboolean output_overlaps (Gde2RROutputInfo *output, Gde2RRConfig *config);
...
static void get_geometry (Gde2RROutputInfo *output, int *w, int *h);
static void apply_configuration_returned_cb (DBusGProxy *proxy, DBusGProxyCall *call_id, void *data);
static gboolean get_clone_size (Gde2RRScreen *screen, int *width, int *height);
static gboolean output_info_supports_mode (App *app, Gde2RROutputInfo *info, int width, int height);
...
static void
on_screen_changed (Gde2RRScreen *scr,
		   gpointer data)
{
    Gde2RRConfig *current;
    App *app = data;

    current = gde2_rr_config_new_current (app->screen, NULL);
    ...
    if (app->labeler) {
	gde2_rr_labeler_hide (app->labeler);
	g_object_unref (app->labeler);
    }

    app->labeler = gde2_rr_labeler_new (app->current_configuration);
...
get_current_modes (App *app)
{
    ...
    if (gde2_rr_config_get_clone (app->current_configuration))
    {
	return gde2_rr_screen_list_clone_modes (app->screen);
    }
    else
    {
	...
	output = gde2_rr_screen_get_output_by_name (app->screen,
	    gde2_rr_output_info_get_name (app->current_output));

	if (!output)
	    return NULL;

	return gde2_rr_output_list_modes (output);
    }
}
...
rebuild_rotation_combo (App *app)
{
    ...
    Gde2RRRotation current;
    int i;

    clear_combo (app->rotation_combo);

    gtk_widget_set_sensitive (app->rotation_combo,
                              app->current_output && gde2_rr_output_info_is_active (app->current_output));

    if (!app->current_output)
	return;

    current = gde2_rr_output_info_get_rotation (app->current_output);
	...
	gde2_rr_output_info_set_rotation (app->current_output, info->rotation);

	/* NULL-GError --- FIXME: we should say why this rotation is not available! */
	if (gde2_rr_config_applicable (app->current_configuration, app->screen, NULL))
	{
    ...
    gde2_rr_output_info_set_rotation (app->current_output, current);
...
rebuild_rate_combo (App *app)
{
	...
	app->refresh_combo, app->current_output && gde2_rr_output_info_is_active (app->current_output));
	...
	gde2_rr_output_info_get_geometry (app->current_output, NULL, NULL, &output_width, &output_height);

	width = gde2_rr_mode_get_width (mode);
	height = gde2_rr_mode_get_height (mode);
	rate = gde2_rr_mode_get_freq (mode);
    ...
    if (!combo_select (app->refresh_combo, idle_free (make_rate_string (gde2_rr_output_info_get_refresh_rate (app->current_output)))))
...
{
    int i, count = 0;
    Gde2RROutputInfo **outputs = gde2_rr_config_get_outputs (app->current_configuration);

    for (i = 0; outputs[i] != NULL; ++i)
    {
	if (gde2_rr_output_info_is_active (outputs[i]))
	    count++;
    }
...
mirror_screens_is_supported (App *app)
{
	...
	for (i = 0; outputs[i] != NULL; i++)
	{
	     ...
	     */
	    if (gde2_rr_output_info_is_connected (outputs[i]) && output_info_supports_mode (app, outputs[i], clone_width, clone_height))
...
rebuild_mirror_screens (App *app)
{
    ...
    mirror_is_active = app->current_configuration && gde2_rr_config_get_clone (app->current_configuration);
...
rebuild_current_monitor_label (App *app)
{
	...
	if (app->current_output)
	{
		...
		tmp = g_strdup_printf (_("Monitor: %s"), gde2_rr_output_info_get_display_name (app->current_output));

	    str = g_strdup_printf ("<b>%s</b>", tmp);
#if GTK_CHECK_VERSION (3, 0, 0)
	    gde2_rr_labeler_get_rgba_for_output (app->labeler, app->current_output, &color);
#else
	    gde2_rr_labeler_get_color_for_output (app->labeler, app->current_output, &color);
#endif
	    use_color = TRUE;
	    g_free (tmp);
	}
...
rebuild_on_off_radios (App *app)
{
    ...
    if (!gde2_rr_config_get_clone (app->current_configuration) && app->current_output)
    {
	if (count_active_outputs (app) > 1 || !gde2_rr_output_info_is_active (app->current_output))
	    sensitive = TRUE;
	else
	    sensitive = FALSE;

	on_active = gde2_rr_output_info_is_active (app->current_output);
...
static void
find_best_mode (Gde2RRMode **modes, int *out_width, int *out_height)
{
    int i;
...
rebuild_resolution_combo (App *app)
{
	...
	|| !app->current_output
	|| !gde2_rr_output_info_is_active (app->current_output))
    {
	...
	width = gde2_rr_mode_get_width (modes[i]);
	height = gde2_rr_mode_get_height (modes[i]);
...
#if 0
    g_debug ("rebuild gui, is on: %d", gde2_rr_output_info_is_active (app->current_output));
#endif
...
#if 0
    g_debug ("sensitive: %d, on: %d", sensitive, gde2_rr_output_info_is_active (app->current_output));
#endif
    gtk_widget_set_sensitive (app->panel_checkbox, sensitive);

    gtk_widget_set_sensitive (app->primary_button, app->current_output && !gde2_rr_output_info_get_primary(app->current_output));

    app->ignore_gui_changes = FALSE;
}

static gboolean
get_mode (GtkWidget *widget, int *width, int *height, int *freq, Gde2RRRotation *rot)
{
    GtkTreeIter iter;
    GtkTreeModel *model;
    GtkComboBox *box = GTK_COMBO_BOX (widget);
...
on_rotation_changed (GtkComboBox *box, gpointer data)
{
	...
	gde2_rr_output_info_set_rotation (app->current_output, rotation);
...
on_rate_changed (GtkComboBox *box, gpointer data)
{
	...
	gde2_rr_output_info_set_refresh_rate (app->current_output, rate);
...
select_resolution_for_current_output (App *app)
{
    ...
    int width, height;
    int x, y;
    gde2_rr_output_info_get_geometry (app->current_output, &x, &y, NULL, NULL);

    width = gde2_rr_output_info_get_preferred_width (app->current_output);
    height = gde2_rr_output_info_get_preferred_height (app->current_output);

    if (width != 0 && height != 0)
    {
	gde2_rr_output_info_set_geometry (app->current_output, x, y, width, height);
	return;
    }
...
monitor_on_off_toggled_cb (GtkToggleButton *toggle, gpointer data)
{
    ...
    gde2_rr_output_info_set_active (app->current_output, is_on);
...
static void
realign_outputs_after_resolution_change (App *app, Gde2RROutputInfo *output_that_changed, int old_width, int old_height)
{
    ...
    gde2_rr_output_info_get_geometry (output_that_changed, &x, &y, &width, &height);
    ...
    outputs = gde2_rr_config_get_outputs (app->current_configuration);
	...
	if (outputs[i] == output_that_changed || gde2_rr_output_info_is_connected (outputs[i]))
	  continue;

	gde2_rr_output_info_get_geometry (outputs[i], &output_x, &output_y, &output_width, &output_height);
	...
	gde2_rr_output_info_set_geometry (outputs[i], output_x, output_y, output_width, output_height);
    }
}
...
on_resolution_changed (GtkComboBox *box, gpointer data)
{
    ...
    gde2_rr_output_info_get_geometry (app->current_output, &x, &y, &old_width, &old_height);
	...
	if (width == 0 || height == 0)
	    gde2_rr_output_info_set_active (app->current_output, FALSE);
	else
	    gde2_rr_output_info_set_active (app->current_output, TRUE);
    }
...
lay_out_outputs_horizontally (App *app)
{
    ...
    {
	int width, height;
	if (gde2_rr_output_info_is_connected (outputs[i]) &&gde2_rr_output_info_is_active (outputs[i]))
	{
	    gde2_rr_output_info_get_geometry (outputs[i], NULL, NULL, &width, &height);
	    gde2_rr_output_info_set_geometry (outputs[i], x, 0, width, height);
	    x += width;
	}
    }
...
/* FIXME: this function is copied from gde2-settings-daemon/plugins/xrandr/gsd-xrandr-manager.c.
 * Do we need to put this function in gde2-desktop for public use?
 */
static gboolean
...
static gboolean
output_info_supports_mode (App *app, Gde2RROutputInfo *info, int width, int height)
{
    Gde2RROutput *output;
    Gde2RRMode **modes;
    int i;
    ...
    output = gde2_rr_screen_get_output_by_name (app->screen, gde2_rr_output_info_get_name (info));
    if (!output)
	return FALSE;
    ...
    for (i = 0; modes[i]; i++) {
	if (gde2_rr_mode_get_width (modes[i]) == width
	    && gde2_rr_mode_get_height (modes[i]) == height)
	    return TRUE;
    }
...
on_clone_changed (GtkWidget *box, gpointer data)
{
    ...
    gde2_rr_config_set_clone (app->current_configuration, gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (app->clone_checkbox)));
	...
	for (i = 0; outputs[i]; i++) {
		...
		gde2_rr_output_info_set_active (outputs[i], TRUE);
		gde2_rr_output_info_get_geometry (outputs[i], &x, &y, NULL, NULL);
		gde2_rr_output_info_set_geometry (outputs[i], x, y, width, height);
	    }
	}
    }
    else
...
static void
get_geometry (Gde2RROutputInfo *output, int *w, int *h)
{
    Gde2RRRotation rotation;

    if (gde2_rr_output_info_is_active (output))
    {
	gde2_rr_output_info_get_geometry (output, NULL, NULL, w, h);
    }
    else
    {
	*h = gde2_rr_output_info_get_preferred_height (output);
	*w = gde2_rr_output_info_get_preferred_width (output);
    }
   rotation = gde2_rr_output_info_get_rotation (output);
   if ((rotation & GDE2_RR_ROTATION_90) || (rotation & GDE2_RR_ROTATION_270))
   {
        int tmp;
        tmp = *h;
        *h = *w;
...
static void
add_edge (Gde2RROutputInfo *output, int x1, int y1, int x2, int y2, GArray *edges)
{
    Edge e;
...
static void
list_edges_for_output (Gde2RROutputInfo *output, GArray *edges)
{
    int x, y, w, h;

    gde2_rr_output_info_get_geometry (output, &x, &y, &w, &h);
...
static void
list_edges (Gde2RRConfig *config, GArray *edges)
{
    int i;
...
static void
list_snaps (Gde2RROutputInfo *output, GArray *edges, GArray *snaps)
{
    int i;
...
static gboolean
output_is_aligned (Gde2RROutputInfo *output, GArray *edges)
{
...
static void
get_output_rect (Gde2RROutputInfo *output, GdkRectangle *rect)
{
    gde2_rr_output_info_get_geometry (output, &rect->x, &rect->y, &rect->width, &rect->height);
}

static gboolean
output_overlaps (Gde2RROutputInfo *output, Gde2RRConfig *config)
{
    int i;
    ...
    for (i = 0; outputs[i]; ++i)
    {
	if (outputs[i] != output && gde2_rr_output_info_is_connected (outputs[i]))
	{
...
static gboolean
gde2_rr_config_is_aligned (Gde2RRConfig *config, GArray *edges)
{
    int i;
		 ...
		 gpointer data)
{
     ...
     */
    if (!gde2_rr_config_get_clone (app->current_configuration) && get_n_connected (app) > 1)
	...
	{
	    int output_x, output_y;
	    gde2_rr_output_info_get_geometry (output, &output_x, &output_y, NULL, NULL);
	...
	if (foo_scroll_area_is_grabbed (area))
	{
	    ...
	    gde2_rr_output_info_get_geometry (output, &old_x, &old_y, &width, &height);
	    new_x = info->output_x + (event->x - info->grab_x) / scale;
	    new_y = info->output_y + (event->y - info->grab_y) / scale;

	    gde2_rr_output_info_set_geometry (output, new_x, new_y, width, height);
		...
		gde2_rr_output_info_set_geometry (output, new_x + snap->dx, new_y + snap->dy, width, height);

		g_array_set_size (new_edges, 0);
		list_edges (app->current_configuration, new_edges);

		if (gde2_rr_config_is_aligned (app->current_configuration, new_edges))
		{
		...
		{
		    gde2_rr_output_info_set_geometry (output, info->output_x, info->output_y, width, height);
		}
	    }
		  ...
		  cairo_t       *cr)
{
    ...
    gtk_style_context_save (widget_style);
    gtk_style_context_set_state (widget_style, GTK_STATE_FLAG_SELECTED);
    gde2_desktop_gtk_style_get_dark_color (widget_style,
                                           gtk_style_context_get_state (widget_style),
                                           &dark_color);
    gtk_style_context_restore (widget_style);
...
paint_output (App *app, cairo_t *cr, int i)
{
    ...
    Gde2RROutputInfo *output = g_list_nth_data (connected_outputs, i);
    ...
    rotation = gde2_rr_output_info_get_rotation (output);
    if (rotation & GDE2_RR_REFLECT_X)
	cairo_scale (cr, -1, 1);

    if (rotation & GDE2_RR_REFLECT_Y)
	cairo_scale (cr, 1, -1);
...
#if GTK_CHECK_VERSION (3, 0, 0)
    gde2_rr_labeler_get_rgba_for_output (app->labeler, output, &output_color);
    r = output_color.red;
    g = output_color.green;
    b = output_color.blue;
#else
    gde2_rr_labeler_get_color_for_output (app->labeler, output, &output_color);
...
static void
compute_virtual_size_for_configuration (Gde2RRConfig *config, int *ret_width, int *ret_height)
{
    int i;
    int width, height;
...
check_required_virtual_size (App *app)
{
    ...
    gde2_rr_screen_get_ranges (app->screen, &min_width, &max_width, &min_height, &max_height);
...
begin_version2_apply_configuration (App *app, GdkWindow *parent_window, guint32 timestamp)
{
    ...
    app->proxy = dbus_g_proxy_new_for_name (app->connection,
					    ...
					    "/org/gde2/SettingsDaemon/XRANDR",
					    "org.gde2.SettingsDaemon.XRANDR_2");
				 ...
				 void             *data)
{
	...
	if (app->apply_configuration_state == APPLYING_VERSION_2
	    ...
	    /* We don't pop up an error message; gde2-settings-daemon already does that
...
sanitize_and_save_configuration (App *app)
{
    ...
    gde2_rr_config_sanitize (app->current_configuration);
    ...
    error = NULL;
    if (!gde2_rr_config_save (app->current_configuration, &error))
    {
...
on_detect_displays (GtkWidget *widget, gpointer data)
{
    ...
    error = NULL;
    if (!gde2_rr_screen_refresh (app->screen, &error)) {
	if (error) {
...
set_primary (GtkWidget *widget, gpointer data)
{
    ...
    outputs = gde2_rr_config_get_outputs (app->current_configuration);
    for (i=0; outputs[i]!=NULL; i++) {
        gde2_rr_output_info_set_primary (outputs[i], outputs[i] == app->current_output);
    }

    gtk_widget_set_sensitive (app->primary_button, !gde2_rr_output_info_get_primary(app->current_output));
}

#define MSD_XRANDR_SCHEMA                 "org.gde2.SettingsDaemon.plugins.xrandr"
...
get_nearest_output (Gde2RRConfig *configuration, int x, int y)
{
    int i;
    int nearest_index;
    int nearest_dist;
    ...
    outputs = gde2_rr_config_get_outputs (configuration);
    for (i = 0; outputs[i] != NULL; i++)
    {
	int dist_x, dist_y;
	int output_x, output_y, output_width, output_height;
 ...
 */
static Gde2RROutputInfo *
get_output_for_window (Gde2RRConfig *configuration, GdkWindow *window)
{
	...
	gde2_rr_output_info_get_geometry (outputs[i], &output_rect.x, &output_rect.y, &output_rect.width, &output_rect.height);

	if (gde2_rr_output_info_is_connected (outputs[i]) && gdk_rectangle_intersect (&win_rect, &output_rect, &intersection))
	{
	    int area;
...
make_default (App *app)
{
    ...
    source_filename = gde2_rr_config_get_intended_filename ();

    command_line = g_strdup_printf ("pkexec %s/gde2-display-properties-install-systemwide %s %s",
				    SBINDIR,
				    source_filename,
				    dest_basename);
...
#define UI_FILE UIDIR "/display-capplet.ui"
    ...
    app->screen = gde2_rr_screen_new (gdk_screen_get_default (), &error);
    ...
    gtk_window_set_default_icon_name ("gde2-preferences-desktop-display");
    gtk_window_set_icon_name (GTK_WINDOW (app->dialog),
			      "gde2-preferences-desktop-display");
