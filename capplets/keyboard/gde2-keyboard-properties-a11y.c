...
#include "gde2-keyboard-properties-a11y.h"
#include <gio/gio.h>
#include "capplet-util.h"

#define NWID(s) GTK_WIDGET (gtk_builder_get_object (notifications_dialog, s))

#define A11Y_SCHEMA "org.gde2.accessibility-keyboard"
...
bell_flash_radio_changed (GtkWidget *widget, GtkBuilder *builder)
{
	...
	int old_bell_flash_type = g_settings_get_enum (finestra_settings, "visual-bell-type");
	...
	if (old_bell_flash_type != new_bell_flash_type)
		g_settings_set_enum (finestra_settings, "visual-bell-type", new_bell_flash_type);
...
notifications_button_clicked_cb (GtkWidget *button, GtkBuilder *dialog)
{
    ...
    gtk_builder_add_from_file (notifications_dialog, GDE2CC_UI_DIR
                               "/gde2-keyboard-properties-a11y-notifications.ui",
                               NULL);
	...
	GSettings *finestra_settings = g_settings_new (FINESTRA_SCHEMA);
	w = NWID ("visual_bell_enable");
	g_settings_bind (finestra_settings, "visual-bell", w, "active", G_SETTINGS_BIND_DEFAULT);
	g_signal_connect (w, "toggled",
			G_CALLBACK (visual_bell_enable_toggled_cb), dialog);
	visual_bell_enable_toggled_cb (w, dialog);

	bell_flash_gsettings_changed (finestra_settings, "visual-bell-type", NULL);
