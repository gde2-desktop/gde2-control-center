...
static void
 xkb_layout_chooser_add_variant_to_available_country_variants
	...
	gchar *utf_variant_name = config_item ?
	    xkb_layout_description_utf8 (gde2kbd_keyboard_config_merge_items
	...
	const gchar *xkb_id =
	    config_item ?
	    gde2kbd_keyboard_config_merge_items (parent_config_item->name,
					      config_item->name) :
	    parent_config_item->name;
						      ...
						      layouts_list)
{
	...
	options_list_strv =
	    gde2kbd_keyboard_config_add_default_switch_option_if_necessary
...
xkb_layout_chooser_print (GtkBuilder * chooser_dialog)
{
	...
	gde2kbd_keyboard_drawing_print (GDE2KBD_KEYBOARD_DRAWING
				     (kbdraw),
				     GTK_WINDOW (CWID
						 ("xkb_layout_chooser")),
				     descr);
			     ...
			     gint response, GtkBuilder * chooser_dialog)
{
	...
	gtk_window_get_size (GTK_WINDOW (dialog), &rect.width,
			     &rect.height);
	gde2kbd_preview_save_position (&rect);
}
...
xkb_layout_choose (GtkBuilder * dialog)
{
	...
	gtk_builder_add_from_file (chooser_dialog, GDE2CC_UI_DIR
	...
	if (gtk_widget_is_toplevel (toplevel)) {
		GdkRectangle *rect = gde2kbd_preview_load_position ();
		if (rect != NULL) {
