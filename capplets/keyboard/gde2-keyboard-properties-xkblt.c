...
{
	char *l, *sl, *v, *sv;
	if (gde2kbd_keyboard_config_get_descriptions
	    (config_registry, visible, &sl, &l, &sv, &v))
		visible = gde2kbd_keyboard_config_format_full_layout (l, v);
...
show_selected_layout (GtkWidget * button, GtkBuilder * dialog)
{
	...
	if (idx != -1) {
		...
		GtkWidget *popup = gde2kbd_keyboard_drawing_new_dialog (idx, descr);
		clear_xkb_elements_list (layouts_list);
		g_free (descr);
	}
}
