...
#include "gde2-keyboard-properties-xkb.h"

#include <libgde2kbd/gde2kbd-desktop-config.h>

#define XKB_GENERAL_SCHEMA "org.gde2.peripherals-keyboard-xkb.general"
#define XKB_KBD_SCHEMA "org.gde2.peripherals-keyboard-xkb.kbd"

XklEngine *engine;
XklConfigRegistry *config_registry;

Gde2kbdKeyboardConfig initial_config;
Gde2kbdDesktopConfig desktop_config;
...
{
	gde2kbd_desktop_config_term (&desktop_config);
	gde2kbd_keyboard_config_term (&initial_config);
...
{
	Gde2kbdKeyboardConfig empty_kbd_config;

	gde2kbd_keyboard_config_init (&empty_kbd_config, engine);
	gde2kbd_keyboard_config_save_to_gsettings (&empty_kbd_config);
	gde2kbd_keyboard_config_term (&empty_kbd_config);
...
setup_xkb_tabs (GtkBuilder * dialog)
{
	...
	gde2kbd_desktop_config_init (&desktop_config, engine);
	gde2kbd_desktop_config_load_from_gsettings (&desktop_config);

	xkl_config_registry_load (config_registry, desktop_config.load_extra_items);

	gde2kbd_keyboard_config_init (&initial_config, engine);
	gde2kbd_keyboard_config_load_from_x_initial (&initial_config, NULL);
...
{
	Gde2kbdKeyboardConfig gswic;
	gboolean enable;

	gde2kbd_keyboard_config_init (&gswic, engine);
	gde2kbd_keyboard_config_load_from_gsettings (&gswic, NULL);

	enable = !gde2kbd_keyboard_config_equals (&gswic, &initial_config);

	gde2kbd_keyboard_config_term (&gswic);
